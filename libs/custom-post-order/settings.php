...
<div class="wrap">
	<h2><?php _e( 'Custom Post Order Settings', 'custom-post-order' ); ?></h2>
	<?php if ( isset( $_GET['msg'] ) ) : ?>
		<div id="message" class="updated below-h2">
			<?php if ( $_GET['msg'] == 'update' ) : ?>
				<p><?php _e( 'Settings Updated.', 'custom-post-order' ); ?></p>
			<?php endif; ?>
		</div>
	<?php endif; ?>
	...
	<form method="post">
		...
		<div id="scporder_select_objects">
			...
			<table class="form-table scpo-table">
				<tbody>
					<tr valign="top">
						<th scope="row"><?php _e( '投稿タイプを並べ替える場合にチェックを入れます。', 'custom-post-order' ); ?></th>
						<td>
							...
							<label>
								...
								<?php _e( 'すべてチェック', 'custom-post-order' ); ?></label><br>
							<?php
		...
		<div id="scporder_select_tags">
			<table class="form-table scpo-table">
				<tbody>
					<tr valign="top">
						<th scope="row"><?php _e( 'タクソノミーを並べ替える場合にチェックを入れます。', 'custom-post-order' ); ?></th>
						<td>
		...
		<div id="scporder_advanved_view">
			<table class="form-table scpo-table">
				<tbody>
				<tr valign="top">
					<th scope="row"><?php _e( 'チェックを入れると投稿タイプの詳細ビューが表示されます。 ', 'custom-post-order' ); ?></th>
					<td>
						<label>
							<div class="epsilon-toggle">
								<input class="epsilon-toggle__input" type="checkbox"
							...
							<?php echo __( '投稿タイプの詳細ビューを表示する。', 'custom-post-order' ); ?>
						</label><br>
						<p class="description">
							<?php _e( '注意: これは上級ユーザーのみを対象としています。', 'custom-post-order' ); ?><br>
							<?php _e( '他のカスタム投稿タイプを含める場合にチェックを入れます。このオプションを有効にした後、設定を更新する必要があります。', 'custom-post-order' ); ?>
						</p>
		...
		<p class="submit">
			<input type="submit" class="button-primary" name="scporder_submit" value="<?php _e( '設定を保存する', 'custom-post-order' ); ?>">
		</p>

	</form>
	<div class="scpo-reset-order">
		<h1>投稿の順序をリセットしますか?</h1>
		<div id="scpo_reset_select_objects">
			<table class="form-table scpo-table">
				<tbody>
				<tr valign="top">
					<th scope="row"><?php _e( '投稿タイプの順序をリセットするにはチェックを入れます。', 'custom-post-order' ); ?></th>
					<td>
						<?php
						foreach ( $post_types as $post_type ) {
							if ( $post_type->name == 'attachment' ) {
		...
		<div>
			<a id="reset-scp-order" class="button button-primary" href="#">順序をリセットする</a>
			<span class="scpo-reset-response"></span>
		</div>
	</div>

	<!-- <h3>Like this plugin?</h3>
	<p>Make sure to <a href="https://wordpress.org/support/plugin/custom-post-order/reviews/?filter=5"><strong>rate it</strong></a> and visit us at <a href="https://colorlib.com/wp/"><strong>Colorlib.com</strong></a></p> -->
