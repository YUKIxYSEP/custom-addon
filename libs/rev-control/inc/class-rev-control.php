...
use Rev_Control\Block_Editor;
use Rev_Control\Singleton;
...
class Rev_Control {
	 ...
	 * @var string
	 */
	private $settings_section = 'rev_control';
	 ...
	 * @var string
	 */
	private $meta_key_limit = '_rev_ctl_limit';
	...
	public function action_admin_init() {
		...
		add_settings_section( $this->settings_section, 'Rev Control', array( $this, 'settings_section_intro' ), $this->settings_page );
		...
		// Bulk actions.
		Rev_Control_Bulk_Actions::get_instance();
	}
	...
	public function settings_section_intro() {
		?>
		<p><?php esc_html_e( 'Set the number of revisions to save for each post type listed. To retain all revisions for a given post type, leave the field empty.', 'rev-control' ); ?></p>
		<p><?php esc_html_e( 'If a post type isn\'t listed, revisions are not enabled for that post type.', 'rev-control' ); ?></p>
		<?php
		...
		if ( $this->plugin_priority() !== $this->priority_default ) :
			...
			<p>
				...
				printf(
					...
					'<code>rev_control_priority</code>'
				);
				?>
			</p>
			<?php
	...
	private function plugin_priority() {
		if ( is_null( self::$priority ) ) {
			$plugin_priority = apply_filters( 'rev_control_priority', $this->priority_default );
	...
	public function filter_wp_revisions_to_keep( $qty, $post ) {
		$post_limit = get_post_meta( $post->ID, REV_CONTROL_LIMIT_META_KEY, true );
	...
	public function action_add_meta_boxes( $post_type, $post ) {
		...
		wp_localize_script(
			...
			array(
				...
				'processing_text' => __( 'Processing&hellip;', 'rev-control' ),
				'ays'             => __( 'Are you sure you want to remove revisions from this post?', 'rev-control' ),
				'autosave'        => __( 'Autosave', 'rev-control' ),
				'nothing_text'    => wpautop( __( 'There are no revisions to remove.', 'rev-control' ) ),
				'error'           => __( 'An error occurred. Please refresh the page and try again.', 'rev-control' ),
			)
		);
	...
	public function revisions_meta_box( $post ) {
		...
		<div id="<?php echo esc_attr( $this->settings_section ); ?>">
			...
			<p class="button purge" data-postid="<?php the_ID(); ?>" data-nonce="<?php echo esc_attr( wp_create_nonce( $this->settings_section . '_purge' ) ); ?>"><?php _e( 'Purge these revisions', 'rev-control' ); ?></p>
	...
	public function ajax_purge() {
		...
		if ( ! $post_id ) {
			$response['error'] = __( 'No post ID was provided. Please refresh the page and try again.', 'rev-control' );
		} elseif ( ! check_ajax_referer( $this->settings_section . '_purge', 'nonce', false ) ) {
			$response['error'] = __( 'Invalid request. Please refresh the page and try again.', 'rev-control' );
		} elseif ( ! current_user_can( 'edit_post', $post_id ) ) {
			$response['error'] = __( 'You are not allowed to edit this post.', 'rev-control' );
		}
	...
	public function action_save_post( $post_id ) {
		...
		if ( isset( $_POST[ $nonce ], $_POST[ $qty ] ) && wp_verify_nonce( sanitize_text_field( $_POST[ $nonce ] ), $this->settings_section . '_limit' ) ) {
			...
			if ( -1 === $limit || empty( $limit ) ) {
				delete_post_meta( $post_id, REV_CONTROL_LIMIT_META_KEY );
			} else {
				update_post_meta( $post_id, REV_CONTROL_LIMIT_META_KEY, absint( $limit ) );
			}
		}
	}
	...
	public function action_admin_head() {
		?>
		<style type="text/css">
			#revisionsdiv-rev-ctl #<?php echo esc_attr( $this->settings_section ); ?> {
				border-top: 1px solid #dfdfdf;
				padding-top: 0;
				margin-top: 20px;
			}

			#revisionsdiv-rev-ctl #<?php echo esc_attr( $this->settings_section ); ?> h4 {
	...
	private function get_post_revisions_to_keep( $post_id ) {
		$to_keep = get_post_meta( $post_id, REV_CONTROL_LIMIT_META_KEY, true );
