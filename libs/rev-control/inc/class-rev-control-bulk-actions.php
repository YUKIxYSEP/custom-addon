...
/**
 * Class Rev_Control_Bulk_Actions.
 */
class Rev_Control_Bulk_Actions {
	use Singleton;
	 ...
	 * @var string
	 */
	protected $action_base = 'rev_ctl_bulk_';
	...
	public function setup() {
		$this->post_types = Rev_Control::get_instance()->get_post_types();
	...
	public function register_admin_hooks() {
		...
		$user_can       = apply_filters(
			'rev_control_current_user_can_bulk_actions',
			$user_can,
			$screen->post_type
		);
	...
	protected function purge_all( $ids ) {
		$plugin = Rev_Control::get_instance();
	...
	public function admin_notices() {
		...
		switch ( str_replace( $this->action_base, '', $message ) ) {
			...
			case 'missing':
				$message = __(
					'Rev Control encountered an unspecified error.',
					'rev-control'
				);
				$type    = 'error';
				break;
